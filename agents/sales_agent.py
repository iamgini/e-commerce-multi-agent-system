...
add_to_cart, remove_from_cart, update_cart_quantity all require a numeric
...
view_cart should present users with the product name and not IDs. Customers 
always refer to products by name, not by ID.

Rule: if you do not already have a product name in the conversation, call
find_product_by_id(product_id) after cart tool. Never guess or invent a product name.

Two-step pattern for every view_cart request:
  1. view_cart(user_id)             -> returns items in the current cart.
  2. find_product_by_id(product_id) -> retrieve product names and use it in your response.

Exception: if the conversation already contains a product_name from the
Recommendation Agent's earlier response, use that name directly without calling
find_product_by_id again.
