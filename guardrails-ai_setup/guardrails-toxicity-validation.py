...
validators = [
    ToxicLanguage(threshold=0.04, validation_method="sentence", on_fail="refrain"),
]

guard = Guard().use(validators=validators)

# Prepare dataset
df = pd.read_csv(f"{os.getcwd()}\\guardrails-ai_setup\\surge_ai\\toxicity_en.csv")
...
df.to_csv(f"{os.getcwd()}\\guardrails-ai_setup\\surge_ai\\toxicty_results_llama3.2_0.07.csv", index=False)
